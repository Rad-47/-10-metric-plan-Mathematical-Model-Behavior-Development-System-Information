...
def normalize_metrics(spiky: Dict) -> Dict[str, float]:
    ...
    # Filler -> inverted cleanliness
    if "filler_ratio" in L:
        out["filler_inv"] = inv100(L.get("filler_ratio"))

    # Sentence length (4..25)
    if "avg_sentence_len" in L:
        out["avg_sentence_len_norm"] = minmax(L.get("avg_sentence_len"), 4.0, 25.0)

    # Patience 0..180
    if "patience" in L:
        out["patience_norm"] = minmax(L.get("patience"), 0.0, 180.0)

    # Keywords average
    if isinstance(L.get("keywords"), dict) and L["keywords"]:
        vals = [to100(v) for v in L["keywords"].values() if to100(v) is not None]
        if vals:
            out["kw_strength"] = sum(vals)/len(vals)

    # Curiosity (optional)
    if "lang_emo_curiosity" in L:
        out["lang_emo_curiosity"] = to100(L.get("lang_emo_curiosity"))
    ...
    if "question_ratio" not in L and "question" in L:
        try:
            ...
            qv = 1.0 if bool(L["question"]) else 0.0
        L["question_ratio"] = qv
    if "question_ratio" in L:
        out["question_ratio"] = to100(L["question_ratio"])

    # Offensiveness inverse
    if isinstance(L.get("offensiveness"), str):
        L["offensiveness"] = 1.0 if L["offensiveness"].lower().startswith("offen") else 0.0
    if "offensiveness" in L:
        out["offensiveness_inv"] = inv100(L["offensiveness"])
    ...
    # Interaction
    if "talk_listen" in I:
        out["talk_balance"] = talk_balance_score(I["talk_listen"])
    if "speed_wpm" in I:
        out["speed_norm"] = minmax(I["speed_wpm"], 90.0, 180.0)

    # High-level
    if "action_items" in H:
        out["action_items"] = to100(H["action_items"])
    if "followup_questions" in H:
        out["followup_questions"] = to100(H["followup_questions"])

    return out

# ---------------- curated subset (10 metrics) ----------------
...
def base_factors(norm: Dict[str, float]) -> Dict[str, float]:
    W = load_weights()
    ...
    for k in bases:
        bases[k] = clamp(bases[k], 0.0, 100.0)
    return bases

def apply_pattern(bases: Dict[str, float], pattern_order: List[str]) -> Dict[str, float]:
    multipliers = load_multipliers()
    pmap = {pattern_order[0]:"primary", pattern_order[1]:"secondary", pattern_order[2]:"tertiary", pattern_order[3]:"quaternary"}
    out = {}
...
def resolve_pattern(pattern_id=None, pattern_name=None, bcat_pattern=None) -> List[str]:
    patterns = load_patterns()
    if bcat_pattern:
        return bcat_pattern
    if pattern_id is not None:
        return patterns.get(str(pattern_id))["order"]
    if pattern_name:
        for v in patterns.values():
            if v["name"].lower() == pattern_name.lower():
                return v["order"]
...
def score_payload(spiky: Dict, pattern_order: List[str]) -> Dict:
    ...
    scores = apply_pattern(bases, pattern_order)
    align = cosine_alignment(scores, pattern_order)
    # drivers
    W = load_weights(); mult = load_multipliers()
    rank = {"primary": mult["primary"], "secondary": mult["secondary"], "tertiary": mult["tertiary"], "quaternary": mult["quaternary"]}
    pmap = {pattern_order[0]:"primary", pattern_order[1]:"secondary", pattern_order[2]:"tertiary", pattern_order[3]:"quaternary"}
    drivers = []
    for m, s in s_cur.items():
        if m not in W:
            continue
        row = W[m]; total = sum(row.values()) or 1.0
        for f, w in row.items():
            contrib = s * (w/total) * rank[pmap[f]]
            drivers.append({"metric": m, "factor": f, "contribution": round(contrib, 2)})
